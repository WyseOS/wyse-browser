var WebBrowser = WebBrowser || (function () {

    /**
     * WebBrowser - A JavaScript module for analyzing web page content and interactive elements
     *
    ...
    // let getInteractiveRects = function () {
    //     // Enhanced visibility check that combines multiple detection methods
    //     function isElementEnhancedVisible(el) {
    //         if (!el) return false;
    ...
    //         // Enhanced overflow checking - check if element is clipped by any ancestor
    //         let parent = el.parentElement;
    //         while (parent && parent !== document.documentElement) {
    //             const parentStyle = window.getComputedStyle(parent);
    //             const parentRect = parent.getBoundingClientRect();

    //             // Check if parent clips content
    ...
    //                 const matrix = new DOMMatrix(parentStyle.transform);
    //                 if (Math.abs(matrix.a) < 0.01 || Math.abs(matrix.d) < 0.01) {
    //                     return false;
    //                 }
    //             }
    ...
    //         // Check if element is within viewport (with some tolerance for partially visible elements)
    //         const viewportWidth = window.innerWidth || document.documentElement.clientWidth;
    //         const viewportHeight = window.innerHeight || document.documentElement.clientHeight;

    //         if (rect.right < -50 || rect.left > viewportWidth + 50 ||
    ...
    //     // Multi-layer strategy: Step 1 - Get standard interactive elements (no Shadow DOM)
    //     function getStandardInteractiveElements() {
    //         let results = [];
    //         let roles = ["scrollbar", "searchbox", "slider", "spinbutton", "switch", "tab", "treeitem",
    //             "button", "checkbox", "gridcell", "link", "menuitem", "menuitemcheckbox",
    //             "menuitemradio", "option", "progressbar", "radio", "textbox", "combobox",
    //             "menu", "tree", "treegrid", "grid", "listbox", "radiogroup", "widget"];
    //         let inertCursors = ["auto", "default", "none", "text", "vertical-text", "not-allowed", "no-drop"];

    //         // Standard interactive elements
    //         let nodeList = document.querySelectorAll("input, select, textarea, button, [href], [onclick], [contenteditable], [tabindex]:not([tabindex='-1'])");
    //         for (let i = 0; i < nodeList.length; i++) {
    //             if (nodeList[i].disabled || !isElementEnhancedVisible(nodeList[i])) {
    //                 continue;
    //             }
    //             results.push(nodeList[i]);
    //         }
    ...
    //         // Elements with interactive cursor styles
    //         nodeList = document.querySelectorAll("*");
    //         for (let i = 0; i < nodeList.length; i++) {
    //             let node = nodeList[i];
    //             if (node.disabled || !isElementEnhancedVisible(node)) {
    //                 continue;
    //             }
    ...
    //     // Multi-layer strategy: Step 2 - Support Shadow DOM elements
    //     function gatherAllInteractiveElements(roles, root = document) {
    //         const elements = [];
    //         const stack = [root];
    //         const selector = roles.join(",");
    ...
    //                 elements.push(...Array.from(currentRoot.querySelectorAll(selector)));
    //             } catch (e) {
    //                 // Ignore invalid selectors in some shadow contexts
    //             }

    //             // Add shadow roots to stack for comprehensive coverage
    //             try {
    //                 currentRoot.querySelectorAll("*").forEach(el => {
    //                     if (el.shadowRoot) {
    //                         stack.push(el.shadowRoot);
    ...
    //                 });
    //             } catch (e) {
    //                 // Some shadow roots may not be accessible
    //             }
    //         }
    ...
    //     // Multi-layer strategy: Step 3 - Combine and filter elements
    //     function getAllInteractiveElements() {
    //         const interactiveRoles = ["input", "select", "textarea", "button", "[href]", "[onclick]",
    //             "[contenteditable]", "[tabindex]:not([tabindex='-1'])", "[role='button']",
    //             "[role='link']", "[role='checkbox']", "[role='radio']", "[role='option']",
    //             "[role='tab']", "[role='menuitem']", "[role='dialog']", "[role='switch']"];

    //         let results = [];

    //         // Get standard elements
    //         let standardElements = getStandardInteractiveElements();
    //         for (let element of standardElements) {
    //             if (results.indexOf(element) === -1) {
    //                 // Enhanced isTopmost check for precise interactivity
    ...
    //         let shadowElements = gatherAllInteractiveElements(interactiveRoles);
    //         shadowElements.forEach(element => {
    //             // Special handling for file inputs and options
    ...
    //             if (element.disabled || !isElementEnhancedVisible(element)) {
    //                 return;
    //             }
    ...
    //     // Label elements with unique IDs for tracking
    //     function labelElementsWithIds(elements) {
    //         for (let i = 0; i < elements.length; i++) {
    //             if (!elements[i].hasAttribute("__elementId")) {
    //                 elements[i].setAttribute("__elementId", "" + (nextLabel++));
    //             }
    ...
    //     // Main execution
    //     let elements = labelElementsWithIds(getAllInteractiveElements());
    //     let results = {};

    //     for (let i = 0; i < elements.length; i++) {
    //         let element = elements[i];
    //         let key = element.getAttribute("__elementId");
    //         let rects = element.getBoundingClientRect();

    //         // Special handling for option elements - enhanced logic
    //         if (element.tagName && element.tagName.toLowerCase() === "option") {
    //             let select_focused = false;
    ...
    //             // Check if option is visible without select being focused
    //             let option_visible = isElementEnhancedVisible(element);

    //             // Check if select is expanded
    //             let select_expanded = false;
    //             if (select) {
    //                 select_expanded = select.hasAttribute("open") || select.hasAttribute("expanded") ||
    //                     (select.size && select.size > 1) || select.multiple;
    //             }

    //             // Check if parent datalist is active
    //             let datalist_active = false;
    //             if (element.closest("datalist")) {
    //                 datalist_active = true;
    //             }

    //             if (!(select_focused || option_visible || select_expanded || datalist_active)) {
    //                 continue;
    //             }
    //         }

    //         let ariaRole = getApproximateAriaRole(element);
    //         let ariaName = getApproximateAriaName(element);
    //         let vScrollable = element.scrollHeight - element.clientHeight >= 1;
    //         let hScrollable = element.scrollWidth - element.clientWidth >= 1;

    //         // Enhanced record with more comprehensive information
    //         let record = {
    //             "tag_name": ariaRole[1],
    //             "role": ariaRole[0],
    //             "aria-name": ariaName,
    //             "v-scrollable": vScrollable,
    //             "h-scrollable": hScrollable,
    //             "is_checked": element.checked !== undefined ? element.checked : null,
    //             "is_disabled": element.disabled || false,
    //             "value": element.value !== undefined ? String(element.value).substring(0, 100) : null,
    //             "text_content": (element.textContent || '').trim().substring(0, 200),
    //             "rects": []
    //         };

    //         // Enhanced rect handling with topmost verification
    //         if (rects && typeof rects.length !== 'undefined' && rects.length > 0) {
    //             for (const rect of rects) {
    ...
    //             }
    //         } else {
    //             // Handle single rect case
    //             if (rects && rects.width > 0 && rects.height > 0) {
    //                 let x = rects.left + rects.width / 2;
    //                 let y = rects.top + rects.height / 2;
    //                 if (isTopmost(element, x, y)) {
    //                     record["rects"].push({
    ...
    //         // Only include elements that have valid rects
