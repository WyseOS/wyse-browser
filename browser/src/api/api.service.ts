...
import { CreateFlowDto, DeployFlowDto, FireFlowDto, GetMetadataDto, UpdateMetadataDto } from './dto/api.flow.dto';
import { BrowserActionDto } from './dto/api.action.dto';
import { Response, responseMessage, responseNotFound, responseBadRequest, responseInternalError } from './dto/response.dto';
import path from 'path';
import fs from 'fs';
import { FlowMetadataPath, WorkletMetadataPath, MaxPagesNumberPerSession, GetDateYYYYMMDD } from '../constants';
...
export class ApiService {
    ...
    private async ensureSessionExists(
        ...
        if (!session_id) {
            ...
            try {
                ...
                this.logger.log(`Created new session for ${context}, session_id: ${newSessionId}`);
    ...
    private loadMetadata(type: MetadataType, name: string): string {
        ...
        if (type === MetadataType.FLOW) {
            resolvePath = path.resolve('../', `${FlowMetadataPath}${name}/manifest.json`);
        } else if (type === MetadataType.WORKLET) {
            resolvePath = path.resolve('../', `${WorkletMetadataPath}${name}/manifest.json`);
        }
    ...
    async createFlow(createFlowDto: CreateFlowDto): Promise<Response> {
        ...
        const { sessionId, error } = await this.ensureSessionExists(createFlowDto.session_id, createFlowDto.is_save_video, createFlowDto.extension_names, "createFlow");
        if (error) {
            return error;
        }

        const id = await this.runtime.addFlow(metadata, sessionId);
        this.logger.log(`Flow created: ${flow_name}, instance_id: ${id}, session_id: ${sessionId}`);
        return responseMessage({ flow_name: flow_name, flow_instance_id: id, session_id: sessionId });
    }

    async deployFlow(deployFlowDto: DeployFlowDto): Promise<Response> {
        const flow = deployFlowDto.flow;
        const { sessionId, error } = await this.ensureSessionExists(deployFlowDto.session_id, deployFlowDto.is_save_video, deployFlowDto.extension_names, "deployFlow");
        if (error) {
            return error;
        }

        const id = await this.runtime.addFlow(JSON.stringify(flow), sessionId);
        this.logger.log(`Flow deployed: ${flow.name}, instance_id: ${id}, session_id: ${sessionId}`);
        return responseMessage({ flow_name: flow.name, flow_instance_id: id, session_id: sessionId });
    }
    ...
    async fireFlow(fireFlowDto: FireFlowDto): Promise<Response> {
        ...
        try {
            const result = await this.runtime.fire(flow_instance_id, action_name, data);
            this.logger.log(`Flow fired: instance_id: ${flow_instance_id}, action: ${action_name}, session_id: ${this.runtime.getSessionId(flow_instance_id)}`);
    ...
    async updateMetadata(type: MetadataType, updateDto: UpdateMetadataDto): Promise<Response> {
        if (type === MetadataType.FLOW) {
            ...
            if (metadata === null) {
                const flowPath = path.resolve('../', `${FlowMetadataPath}${updateDto.name}`);
                fs.mkdirSync(flowPath, { recursive: true });

                // Add README.md
                const readmePath = path.resolve('../', `${FlowMetadataPath}${updateDto.name}/README.md`);
                fs.writeFileSync(readmePath, "# " + updateDto.name);
            }

            // Save file
            const jsonData = JSON.stringify(updateDto.data);
            const filePath = path.resolve('../', `${FlowMetadataPath}${updateDto.name}/manifest.json`);
            fs.writeFileSync(filePath, jsonData);

            return responseMessage({ metadata_type: type, name: updateDto.name, data: updateDto.data });
        } else {
            return responseNotFound({
                metadata_type: type,
                name: "",
    ...
    async getDefaultSession(): Promise<Response> {
        ...
        if (!session) {
            ...
            return responseNotFound({ session_id: "", result: "Error: Default session not found" });
        }
    ...
    async createSession(createSessionDto: CreateSessionDto): Promise<Response> {
        try {
            const sessionContext = SessionContext.FromJson(JSON.stringify(createSessionDto));
            const sessionId = await this.runtime.createSession(sessionContext, createSessionDto.session_id);
    ...
    getSessionContext(sessionId: string): Response {
        ...
        try {
            ...
            this.logger.log(`Retrieved session context for ID ${sessionId}: ${JSON.stringify(contextJson, null, 2)}`);
    ...
    async listSessions(): Promise<Response> {
        ...
        return responseMessage({
            sessions: await Promise.all(sessions.map(async session => await session.toJson()))
        });
    }
    ...
    async screenshot(sessionId: string): Promise<Response> {
        ...
        try {
            ...
            this.logger.error(`Screenshot failed, session: ${sessionId}, error: ${error.message}`);
            return responseInternalError({
                timestamp: date,
                session_id: sessionId,
                screenshot: null,
    ...
    async switchPage(sessionId: string, pageId: number): Promise<Response> {
        ...
        try {
            ...
            this.logger.log(`Page switched for session: ${sessionId}, page_id: ${pageId}`);
            return responseMessage({
                session_id: sessionId,
                page_id: pageId,
    ...
    async releasePage(sessionId: string, pageId: number): Promise<Response> {
        ...
        try {
            const pages = await session.getPages();
            if (pageId >= pages.length) {
                return responseMessage({ "result": "The page does not exist", "video_url": "" });
            }
            ...
            if (session.isSaveVideo && videoPath) {
                try {
                    let filename = "screenshot/" + today + "/" + sessionId + "/video_" + path.basename(videoPath);
            ...
            this.logger.log(`Page released for session: ${sessionId}, page_id: ${pageId}`);
    ...
    async browserAction(browserActionDto: BrowserActionDto): Promise<Response> {
        ...
        try {
            const result = await this.runtime.browserAction(session_id, page_id, action_name, data);
            return responseMessage({
                session_id: session_id,
                page_id: page_id,
                action_name: action_name,
            ...
            this.logger.error(`Action failed, session: ${session_id}, page: ${page_id}, action: ${action_name}, error: ${error.message}`);
            return responseInternalError({
                session_id: session_id,
                page_id: page_id,
                action_name: action_name,
    ...
    async batchAction(batchActionDto: BatchActionsDto): Promise<Response> {
        ...
        try {
            this.logger.log(`Start Batch Actions, session: ${session_id}, page: ${page_id}, actions: ${JSON.stringify(batchActionDto.actions)}`);
            const results = await this.runtime.batchBrowserAction(session_id, page_id, batchActionDto.actions);
