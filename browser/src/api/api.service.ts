...
import { CreateProxyDto } from './dto/api.proxy.dto';
...
export class ApiService {
    ...
    async createProxy(createProxy: CreateProxyDto): Promise<Response> {
        const proxy_name = createProxy.proxy_name;
        const loadProfile = this.loadMetadata(MetadataType.PROXY, proxy_name);
        if (loadProfile === null) {
            const proxyPath = path.resolve(
                '../',
                `${ProxyMetadataPath}${proxy_name}`,
            );
            fs.mkdirSync(proxyPath, { recursive: true });
            const readmePath = path.resolve(
                '../',
                `${ProxyMetadataPath}${proxy_name}/README.md`,
            );
            fs.writeFileSync(readmePath, '# ' + proxy_name);
        }

        const jsonData = JSON.stringify(createProxy);
        const filePath = path.resolve(
            '../',
            `${ProxyMetadataPath}${proxy_name}/manifest.json`,
        );
    ...
    async createProfile(data): Promise<Response> {
        console.debug('data', data);
        const { profile_name } = data;
        const loadCurrent = this.loadMetadata(MetadataType.PROFILE, profile_name);
        if (loadCurrent === null) {
            const currentPath = path.resolve(
                '../',
                `${ProfileMetadataPath}${profile_name}`,
            );
            fs.mkdirSync(currentPath, { recursive: true });
            const readmePath = path.resolve(
                '../',
                `${ProfileMetadataPath}${profile_name}/README.md`,
            );
            fs.writeFileSync(readmePath, '# ' + profile_name);
        }

        const jsonData = JSON.stringify(data);
        const filePath = path.resolve(
            '../',
            `${ProfileMetadataPath}${profile_name}/manifest.json`,
        );
        fs.writeFileSync(filePath, jsonData);
        return responseMessage({
