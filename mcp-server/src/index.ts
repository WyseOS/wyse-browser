...
import { Runtime, BrowserAction, SessionContext } from 'wyse-browser';
...
export async function executeBrowserAction(actionName: string, params: Record<string, any> = {}): Promise<string> {
    try {
        ...
        throw new Error(`Failed to execute ${actionName}: ${(error as Error).message}`);
    }
}
...
async function main() {
    try {
        ...
        console.error("Wyse Browser MCP server is running...");
    } catch (error) {
